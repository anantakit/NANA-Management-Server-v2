...
const (
	...
	// FailureCodeStaleAfterRecovery — a meter correction was recorded after this
	// bill was generated, so it does not reflect the latest meter truth (Q1.6
	// freshness gate). Business-rule skip, not a system error: admin must
	// regenerate (VOID + GENERATE) before this bill can finalize.
	FailureCodeStaleAfterRecovery BatchFinalizeFailureCode = "STALE_AFTER_RECOVERY"
)
