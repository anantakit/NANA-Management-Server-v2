...
Phase 1A read-only Audit + Phase 1B per-room decision storage. Trust tool (not productivity tool): the operator question is "ระบบจัดห้องเข้ากลุ่มถูกไหม / ตัดสินได้และเปลี่ยนใจได้", not "ออกบิลได้เร็วแค่ไหน". Bulk is 1C, Generate is 1D.
