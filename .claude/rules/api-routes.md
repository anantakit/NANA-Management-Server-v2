...
| POST | `/:id/regenerate-draft` | **Monthly Draft Refresh (Epic A).** Atomically VOID a stale MONTHLY DRAFT (predates a Reading Recovery, blocked by the freshness gate) + regenerate a fresh DRAFT from source-of-truth via the SHARED `buildMonthlyDraftBill` path (same generation as CreateMonthlyBill / reconciliation generate) so it picks up the S0-gated recovery forward credit. One TX, row-locked (void-old before create-new for the partial unique index). No body. MONTHLY DRAFT only — SETTLEMENT + FINALIZED/PAID/VOID rejected 400 (settlement recovery reconciliation is a separate workflow, S4). Voids old (void_reason=REGENERATED) + emits VOID + CREATE_DRAFT audits. Returns 201 with the new DRAFT. |
