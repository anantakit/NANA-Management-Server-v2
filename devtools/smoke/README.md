...
| `make smoke-recovery-survives-cancel` | Epic B — "Recovery survives Move-out cancel" capability (self-seeding browser): over-record via UI → settlement Model B → cancel move-out → monthly refunds the over-charge exactly once → no duplicate recovery. Locks the owner invariant that cancel doesn't erase meter truth |
...
npm run smoke:recovery-survives-cancel           # SMOKE_HEADED=1 to watch
```
